...
import { getUnpaidVisits, getPatient } from '@/lib/firestore';
...
export default function KasirPage() {
  ...
  useEffect(() => {
    ...
    if (!authLoading && appUser && appUser.role !== 'admin' && appUser.role !== 'kasir') {
      router.push('/');
    }
  ...
  const loadUnpaidVisits = async () => {
    setLoading(true);
    try {
      const data = await getUnpaidVisits();
      setVisits(data);
  ...
  const totalUnpaid = visits.reduce((sum, visit) => sum + visit.totalBiaya, 0);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Kelola pembayaran pasien</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <Card>
            <div className="text-center">
              <p className="text-gray-600 text-sm mb-2">Belum Bayar</p>
              <p className="text-4xl font-bold text-red-600">{visits.length}</p>
            </div>
          </Card>
          <Card className="md:col-span-2">
            <div className="text-center">
              <p className="text-gray-600 text-sm mb-2">Total Piutang</p>
                  ...
                  {visits.map((visit) => {
                    ...
                    return (
                      <tr key={visit.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-sm">
                          <Link href={`/kasir/visit/${visit.id}`}>
